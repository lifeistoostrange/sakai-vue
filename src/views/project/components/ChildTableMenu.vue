...
<template>
    <div class="card">
        <div class="font-semibold text-xl mb-4">하위 메뉴 목록</div>
        <DataTable :value="customers1" :paginator="true" :rows="16" dataKey="id" :rowHover="true" scrollable
            scrollHeight="57rem" v-model:filters="filters1" filterDisplay="menu" :loading="loading1" :filters="filters1"
            :globalFilterFields="['name', 'country.name', 'representative.name', 'balance', 'status']" showGridlines>
            <template #header>
                <div class="flex justify-between">
                    <div>
                        <Button type="button" label="등록" text />
                        <Button type="button" label="수정" outlined @click="clearFilter()" />
            ...
            <Column header="상위메뉴명" filterField="date" dataType="date" style="min-width: 10rem">
                <template #body="{ data }">
